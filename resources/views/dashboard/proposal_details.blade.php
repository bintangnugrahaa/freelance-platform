            ...
            <div class="bg-white shadow-lg rounded-2xl p-8 flex flex-col gap-y-6 transition-all duration-300">
                @if ($project->has_finished)
                    @if ($projectApplicant->status == 'Hired')
                        <span class="text-sm font-medium bg-green-100 text-green-700 rounded-lg p-4 w-full">
                            Project completed and revenue added to your Wallet
                        </span>
                    @else
                        <span class="text-sm font-medium bg-yellow-100 text-yellow-700 rounded-lg p-4 w-full">
                            Project completed, please apply to other available projects
                        </span>
                    @endif
                @else
                    <span class="text-sm font-medium bg-blue-100 text-blue-700 rounded-lg p-4 w-full">
                        Project is still in development
                    </span>
                @endif

                <h3 class="text-gray-900 text-xl font-semibold tracking-tight">You've applied to this job</h3>
                ...
                <div
                    ...
                    <div class="flex flex-row items-center gap-x-4">
                        <img src="{{ Storage::url($project->thumbnail ?? 'default-thumbnail.png') }}"
                            alt="{{ $project->name }}" class="rounded-2xl object-cover w-24 h-16 shadow-sm">
                ...
                <p class="text-gray-500 text-sm font-medium">{{ $projectApplicant->created_at->format('d M Y') }}</p>
                ...
                @if ($projectApplicant->status == 'Hired')
                    ...
                    <div class="flex flex-row items-center gap-x-4">
                        ...
                        <div>
                            <h3 class="text-gray-900 text-lg font-semibold tracking-tight">You're Hired!</h3>
                            <p class="text-gray-500 text-sm font-medium">
                                The client will invite you via email to discuss in a Zoom Meeting.
                            </p>
                        </div>
                    </div>

                    <hr class="my-6 border-gray-200">
                    <h3 class="text-gray-900 text-xl font-semibold tracking-tight">Contact Client</h3>
                    <div
                        ...
                        <p class="text-gray-900 text-sm font-semibold">{{ $projectApplicant->project->owner->email }}
                        </p>
                    </div>
                    ...
                    <div class="flex flex-row items-center gap-x-4">
                        ...
                        <div>
                            <h3 class="text-gray-900 text-lg font-semibold tracking-tight">Income Rp
                                {{ number_format($project->budget, 0, ',', '.') }}</h3>
                            <p class="text-gray-500 text-sm font-medium">
                                Revenue will be added to your Wallet once the project is completed.
                            </p>
                        </div>
                    </div>
                @elseif ($projectApplicant->status == 'Waiting')
                    <!-- No additional content for Waiting status -->
                @elseif ($projectApplicant->status == 'Rejected')
                    <hr class="my-6 border-gray-200">
                    <div class="flex flex-row items-center gap-x-4">
                        ...
                        <div>
                            <h3 class="text-gray-900 text-lg font-semibold tracking-tight">Proposal Rejected</h3>
                            <p class="text-gray-500 text-sm font-medium">

    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-2xl p-8 flex flex-col gap-y-6 transition-all duration-300">
                <span class="text-md font-medium bg-red-100 text-red-700 rounded-lg p-4 w-fit">
                    Penarikan saldo hanya bisa dilakukan minimal jika mencapai Rp100.000
                </span>
                <div
                    class="flex flex-row justify-between items-center p-6 bg-gray-50 rounded-xl border border-gray-100 hover:shadow-md transition-shadow duration-300">
                ...
                <h3 class="text-gray-900 text-xl font-semibold tracking-tight">Withdraw Money</h3>
                <form method="POST" action="{{ route('dashboard.wallet.withdraw.store') }}"
                    enctype="multipart/form-data">
                    @csrf

                    <div>
                        <x-input-label for="bank_name" :value="__('Bank Name')" class="text-gray-700 font-medium" />
                        <x-text-input id="bank_name"
                            ...
                            type="text" name="bank_name" :value="old('bank_name')" required autofocus
                            autocomplete="bank_name" />
                        <x-input-error :messages="$errors->get('bank_name')" class="mt-2 text-red-600" />
                    </div>

                    <div class="mt-6">
                        <x-input-label for="bank_account_name" :value="__('Bank Account Name')" class="text-gray-700 font-medium" />
                        <x-text-input id="bank_account_name"
                            class="block mt-1 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                            type="text" name="bank_account_name" :value="old('bank_account_name')" required
                            autocomplete="bank_account_name" />
                        <x-input-error :messages="$errors->get('bank_account_name')" class="mt-2 text-red-600" />
                    </div>

                    <div class="mt-6">
                        <x-input-label for="bank_account_number" :value="__('Bank Account Number')" class="text-gray-700 font-medium" />
                        <x-text-input id="bank_account_number"
                            class="block mt-1 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                            type="text" name="bank_account_number" :value="old('bank_account_number')" required
                            autocomplete="bank_account_number" />
                        <x-input-error :messages="$errors->get('bank_account_number')" class="mt-2 text-red-600" />
                    </div>

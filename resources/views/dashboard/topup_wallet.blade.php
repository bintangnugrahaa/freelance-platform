    ...
    <div class="py-12 bg-gray-50 min-h-screen">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow-lg rounded-2xl p-8 flex flex-col gap-y-6 transition-all duration-300">
                ...
                <div>
                    <h3 class="text-gray-900 text-xl font-semibold tracking-tight mb-4">Send Payment to:</h3>
                    <div
                        class="flex flex-col sm:flex-row gap-x-6 gap-y-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                        <div>
                            <p class="text-gray-500 text-sm font-medium">Bank</p>
                            <h3 class="text-gray-900 text-lg font-semibold tracking-tight">BCA</h3>
                        </div>
                        <div>
                            <p class="text-gray-500 text-sm font-medium">No Account</p>
                            <h3 class="text-gray-900 text-lg font-semibold tracking-tight">7650542112</h3>
                        </div>
                        <div>
                            <p class="text-gray-500 text-sm font-medium">Account Name</p>
                            <h3 class="text-gray-900 text-lg font-semibold tracking-tight">PT Nanuraga Jaya Abadi</h3>
                        </div>
                    </div>
                </div>
                ...
                <h3 class="text-gray-900 text-xl font-semibold tracking-tight">Confirm Topup Wallet</h3>
                <form method="POST" action="{{ route('dashboard.wallet.topup.store') }}" enctype="multipart/form-data">
                    @csrf

                    <div>
                        <x-input-label for="amount" :value="__('Amount')" class="text-gray-700 font-medium" />
                        <x-text-input id="amount"
                            class="block mt-1 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                            type="text" name="amount" :value="old('amount')" required autofocus autocomplete="amount" />
                        <x-input-error :messages="$errors->get('amount')" class="mt-2 text-red-600" />
                    </div>
                    ...
                    <div class="mt-6">
                        ...
                        <x-text-input id="proof"
                            class="block mt-1 w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500"
                            type="file" name="proof" required autocomplete="proof" />
